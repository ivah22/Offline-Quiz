...
function Preview({
  quiz, selected, onSelect, onStart, isNew
}: { quiz: Quiz; selected: number | null; onSelect: (n: number) => void; onStart: () => void; isNew?: boolean }) {
  const settings = useSettings();
  const navigate = useNavigate();
  ...
  return (
    <div className="min-h-screen pb-16">
      <header className="mx-auto max-w-2xl px-5 pt-8">
        <a href="/" onClick={handleBack} className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
          <ArrowLeft className="size-4" /> {isNew ? "Cancel Quiz" : "Library"}
        </a>
      </header>
      <main className="mx-auto max-w-2xl px-5 pt-6">
        ...
        <button onClick={handleMainAction}
          className="mt-5 w-full animate-neon-pulse rounded-2xl bg-gradient-primary py-4 text-base font-semibold text-primary-foreground shadow-soft active:scale-[0.98] hover-glow">
          {isNew ? "Upload Quiz" : "Start quiz"}
        </button>
      </main>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will discard the newly uploaded quiz. Are you sure?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>No</AlertDialogCancel>
            <AlertDialogAction onClick={handleCancelQuiz}>
              Yes, Cancel
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
            ...
            {currentChoices.map((c, i) => {
              ...
              return (
                <button key={i} disabled={revealedNow}
                  onClick={() => pick(c)}
                  className={`flex w-full hover-glow items-center gap-4 rounded-2xl border-2 p-4 text-left transition active:scale-[0.98] disabled:cursor-default ${cls}`}>
